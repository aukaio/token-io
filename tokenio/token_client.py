...
class TokenClient:
    ...
    def create_member(self, alias=None, member_type=PERSONAL):
        ...
        member_id = self._unauthenticated_client.create_member_id(member_type=member_type)
        crypto_engine = self.CryptoEngine(member_id)
        operations = []
        metadata = []
        operations.append(utils.create_add_key_member_operation(crypto_engine.generate_key(Key.PRIVILEGED)))
        operations.append(utils.create_add_key_member_operation(crypto_engine.generate_key(Key.STANDARD)))
        operations.append(utils.create_add_key_member_operation(crypto_engine.generate_key(Key.LOW)))
        ...
        created_member = self._unauthenticated_client.create_member(member_id=member_id, operations=operations,
                                                                    metadata=metadata, signer=signer)
        # assert created_member.id == member_id
        crypto_engine = self.CryptoEngine(created_member.id)
        client = Client.authenticated(self.channel, created_member.id, crypto_engine)
    ...
    def get_banks(self, ids=None, search=None, country=None, page=None, per_page=None, sort=None,
                  provider=None):
        return self._unauthenticated_client.get_banks(ids, search, country, page, per_page, sort, provider)
    ...
    def generate_token_request_url(self, request_id, state='', csrf_token=''):
        ...
        return self.TOKEN_REQUEST_TEMPLATE.format(url=self.config.web_url, state=token_request_state.serialize(),
    ...
    def get_recovery_authorization(self, verification_id, code, privileged_key):
        return self._unauthenticated_client.get_recovery_authorization(verification_id, code, privileged_key)
