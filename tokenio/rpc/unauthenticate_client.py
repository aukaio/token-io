...
class UnauthenticatedClient:
    ...
    def create_member_id(self, member_type,
    ...
    def get_banks(self, ids=None, search=None, country=None, page=None, per_page=None, sort=None,
                  provider=None) -> GetBanksResponse:
    ...
    def complete_recovery_with_default_rule(self, member_id, verification_id, code, crypto_engine: CryptoEngine):
    ...
    def get_recovery_authorization(self, verification_id, code, privileged_key):
        request = CompleteRecoveryRequest(verification_id=verification_id, code=code, key=privileged_key)
        with self._unauthenticated_channel as channel:
            response = channel.stub.CompleteRecovery(request)
        # TODO: VerificationException
        return response.recovery_entry

    def complete_recovery(self, member_id, recovery_operations, privileged_key, crypto_engine: CryptoEngine):
        standard_key = crypto_engine.generate_key(Key.STANDARD)
        low_key = crypto_engine.generate_key(Key.LOW)
        ...
        operations.extend(utils.to_add_key_operations([privileged_key, standard_key, low_key]))
