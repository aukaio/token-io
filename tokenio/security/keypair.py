...
class KeyPair:
    def __init__(self, verifying_key, signing_key=None, level=None, expires_at_ms=None):
    ...
    def generate_key(cls, level, expiration_ms=None):
        ...
        return cls(verifying_key=verifying_key, signing_key=signing_key, level=level,
    ...
    def sign(self, data: bytes) -> str:
        if self.signing_key is None:
            raise MissingSigningKeyError("KeyPair does not contain signing_key.")
    ...
    def to_storable_dict(self):
        return {
            ...
            'level': self.level,
            'verifying_key': base64.urlsafe_b64encode(self.verifying_key.to_bytes()).decode(),
    ...
    def generate_key_from_storable_dict(cls, data):
        verifying_key = ed25519.VerifyingKey(base64.urlsafe_b64decode(data['verifying_key'].encode()))
        signing_key = ed25519.SigningKey(base64.urlsafe_b64decode(data['signing_key'].encode()))
    ...
    def to_key(self):
        return Key(id=self.id, level=self.level, algorithm=self.algorithm,
                   public_key=base64.urlsafe_b64encode(self.verifying_key.to_bytes()).decode().strip('='))
...
def _generate_key_id(verifying_key):
    return base64.urlsafe_b64encode(hashlib.sha256(verifying_key.to_bytes()).digest()).decode()[:16]
